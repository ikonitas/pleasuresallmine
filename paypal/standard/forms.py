    ...
    def __init__(self, cart, button_type="buy", *args, **kwargs):
        ...
        if order.delivery.price:
            self.fields['handling_cart'] = forms.CharField(
                    ...
                    initial=str(order.delivery.price)
            )
        for i, line in enumerate(cart):
            self.fields['amount_'+str(i+1)] = forms.CharField(widget=forms.HiddenInput(), initial=str(line.item.total_price()))
            self.fields['item_name_'+str(i+1)] = forms.CharField(widget=forms.HiddenInput(), initial=str(line.item.name))
            self.fields['quantity_'+str(i+1)] = forms.CharField(
                widget=forms.HiddenInput(),
                initial=str(line.quantity))
